...
func TestGetLocalSyncBranch_AlwaysEmpty(t *testing.T) {
	// sync-branch config support has been removed; getLocalSyncBranch always returns "".
	got := getLocalSyncBranch("/nonexistent")
	if got != "" {
		t.Errorf("getLocalSyncBranch() = %q, want empty string", got)
	}
}
