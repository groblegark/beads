...
// nudgeCmd sends a message to an agent's inbox, waking it if blocked on "bd yield".
// This is the send half of the yield/nudge pair: nudge sends, yield receives. (bd-pxogr)
var nudgeCmd = &cobra.Command{
	Use:   "nudge <agent> <message>",
	Short: "Send a message to an agent (wakes bd yield)",
	Long: `Send a message to an agent's inbox, waking it if blocked on "bd yield".
...
persisted) and triggers a NATS publish that unblocks "bd yield" in real-time.
