...
func isAllowedUpdateField(key string) bool {
	allowed := map[string]bool{
		...
		"due_at": true, "defer_until": true, "await_type": true, "await_id": true,
	}
	return allowed[key]
}
...
// hq-b0b22c.3: Add decision point methods for decision workflow support
// =============================================================================
...
func (s *DoltStore) CreateDecisionPoint(ctx context.Context, dp *types.DecisionPoint) error {
	...
	// Verify issue exists
	var count int
	err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM issues WHERE id = ?", dp.IssueID).Scan(&count)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx, `
		...
		WHERE id = ? AND (await_type IS NULL OR await_type = '' OR await_type != 'decision')
	`, dp.IssueID)
	if err != nil {
