package daemon

import (
	"context"
	"encoding/json"
	"fmt"
	"os"
	"sync/atomic"
	"time"

	"github.com/redis/go-redis/v9"
	"github.com/steveyegge/beads/internal/types"
)

const (
	defaultRedisNamespace = "bd"
	defaultRedisWispTTL   = 24 * time.Hour
)

// RedisWispOption is a functional option for configuring a RedisWispStore.
type RedisWispOption func(*redisWispStore)

// WithNamespace sets the Redis key namespace.
func WithNamespace(ns string) RedisWispOption {
	return func(s *redisWispStore) {
		s.namespace = ns
	}
}

// WithTTL sets the TTL for wisp keys.
func WithTTL(ttl time.Duration) RedisWispOption {
	return func(s *redisWispStore) {
		s.ttl = ttl
	}
}

// redisWispStore implements WispStore backed by Redis.
type redisWispStore struct {
	client    *redis.Client
	namespace string
	ttl       time.Duration
	closed    atomic.Bool
}

// NewRedisWispStore creates a new Redis-backed WispStore.
// The redisURL should be a valid Redis URL (e.g., "redis://localhost:6379/0").
// Connectivity is verified via Ping on creation.
func NewRedisWispStore(redisURL string, opts ...RedisWispOption) (WispStore, error) {
	ropts, err := redis.ParseURL(redisURL)
	if err != nil {
		return nil, fmt.Errorf("invalid redis URL: %w", err)
	}

	client := redis.NewClient(ropts)

	s := &redisWispStore{
		client:    client,
		namespace: defaultRedisNamespace,
		ttl:       defaultRedisWispTTL,
	}

	// Apply env var overrides first
	if ns := os.Getenv("BD_REDIS_NAMESPACE"); ns != "" {
		s.namespace = ns
	}
	if ttlStr := os.Getenv("BD_REDIS_WISP_TTL"); ttlStr != "" {
		if d, err := time.ParseDuration(ttlStr); err == nil {
			s.ttl = d
		}
	}

	// Apply functional options (override env vars)
	for _, opt := range opts {
		opt(s)
	}

	// Verify connectivity
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if err := client.Ping(ctx).Err(); err != nil {
		client.Close()
		return nil, fmt.Errorf("redis ping failed: %w", err)
	}

	return s, nil
}

// wispKey returns the Redis key for a wisp by ID.
func (s *redisWispStore) wispKey(id string) string {
	return s.namespace + ":wisp:" + id
}

// indexKey returns the Redis key for the wisp index set.
func (s *redisWispStore) indexKey() string {
	return s.namespace + ":wisp:index"
}

// Create adds a new wisp to the store.
func (s *redisWispStore) Create(ctx context.Context, issue *types.Issue) error {
	if s.closed.Load() {
		return fmt.Errorf("wisp store is closed")
	}

	if issue == nil {
		return fmt.Errorf("issue cannot be nil")
	}

	if issue.ID == "" {
		return fmt.Errorf("issue ID cannot be empty")
	}

	// Check for duplicate
	exists, err := s.client.SIsMember(ctx, s.indexKey(), issue.ID).Result()
	if err != nil {
		return fmt.Errorf("redis check duplicate: %w", err)
	}
	if exists {
		return fmt.Errorf("wisp %s already exists", issue.ID)
	}

	// Clone to prevent mutation of caller's issue
	clone := cloneIssue(issue)
	clone.Ephemeral = true

	now := time.Now()
	if clone.CreatedAt.IsZero() {
		clone.CreatedAt = now
	}
	if clone.UpdatedAt.IsZero() {
		clone.UpdatedAt = now
	}

	data, err := json.Marshal(clone)
	if err != nil {
		return fmt.Errorf("marshal wisp: %w", err)
	}

	pipe := s.client.Pipeline()
	pipe.Set(ctx, s.wispKey(clone.ID), data, s.ttl)
	pipe.SAdd(ctx, s.indexKey(), clone.ID)
	_, err = pipe.Exec(ctx)
	if err != nil {
		return fmt.Errorf("redis create: %w", err)
	}

	return nil
}

// Get retrieves a wisp by ID.
func (s *redisWispStore) Get(ctx context.Context, id string) (*types.Issue, error) {
	if s.closed.Load() {
		return nil, fmt.Errorf("wisp store is closed")
	}

	data, err := s.client.Get(ctx, s.wispKey(id)).Bytes()
	if err == redis.Nil {
		return nil, nil // Not found is not an error
	}
	if err != nil {
		return nil, fmt.Errorf("redis get: %w", err)
	}

	var issue types.Issue
	if err := json.Unmarshal(data, &issue); err != nil {
		return nil, fmt.Errorf("unmarshal wisp: %w", err)
	}

	return &issue, nil
}

// List returns wisps matching the filter.
func (s *redisWispStore) List(ctx context.Context, filter types.IssueFilter) ([]*types.Issue, error) {
	if s.closed.Load() {
		return nil, fmt.Errorf("wisp store is closed")
	}

	ids, err := s.client.SMembers(ctx, s.indexKey()).Result()
	if err != nil {
		return nil, fmt.Errorf("redis list index: %w", err)
	}

	if len(ids) == 0 {
		return nil, nil
	}

	// Build keys for MGET
	keys := make([]string, len(ids))
	for i, id := range ids {
		keys[i] = s.wispKey(id)
	}

	vals, err := s.client.MGet(ctx, keys...).Result()
	if err != nil {
		return nil, fmt.Errorf("redis mget: %w", err)
	}

	var results []*types.Issue
	for _, val := range vals {
		if val == nil {
			continue
		}
		str, ok := val.(string)
		if !ok {
			continue
		}
		var issue types.Issue
		if err := json.Unmarshal([]byte(str), &issue); err != nil {
			continue // skip corrupt entries
		}
		if matchesFilter(&issue, filter) {
			results = append(results, &issue)
		}
	}

	if filter.Limit > 0 && len(results) > filter.Limit {
		results = results[:filter.Limit]
	}

	return results, nil
}

// Update modifies an existing wisp.
func (s *redisWispStore) Update(ctx context.Context, issue *types.Issue) error {
	if s.closed.Load() {
		return fmt.Errorf("wisp store is closed")
	}

	if issue == nil {
		return fmt.Errorf("issue cannot be nil")
	}

	if issue.ID == "" {
		return fmt.Errorf("issue ID cannot be empty")
	}

	// Check existence
	exists, err := s.client.SIsMember(ctx, s.indexKey(), issue.ID).Result()
	if err != nil {
		return fmt.Errorf("redis check exists: %w", err)
	}
	if !exists {
		return fmt.Errorf("wisp %s not found", issue.ID)
	}

	clone := cloneIssue(issue)
	clone.Ephemeral = true
	clone.UpdatedAt = time.Now()

	data, err := json.Marshal(clone)
	if err != nil {
		return fmt.Errorf("marshal wisp: %w", err)
	}

	// SET with refreshed TTL
	if err := s.client.Set(ctx, s.wispKey(clone.ID), data, s.ttl).Err(); err != nil {
		return fmt.Errorf("redis update: %w", err)
	}

	return nil
}

// Delete removes a wisp by ID.
func (s *redisWispStore) Delete(ctx context.Context, id string) error {
	if s.closed.Load() {
		return fmt.Errorf("wisp store is closed")
	}

	// Check existence
	exists, err := s.client.SIsMember(ctx, s.indexKey(), id).Result()
	if err != nil {
		return fmt.Errorf("redis check exists: %w", err)
	}
	if !exists {
		return fmt.Errorf("wisp %s not found", id)
	}

	pipe := s.client.Pipeline()
	pipe.Del(ctx, s.wispKey(id))
	pipe.SRem(ctx, s.indexKey(), id)
	_, err = pipe.Exec(ctx)
	if err != nil {
		return fmt.Errorf("redis delete: %w", err)
	}

	return nil
}

// Count returns the number of wisps in the store.
func (s *redisWispStore) Count() int {
	ctx := context.Background()
	n, err := s.client.SCard(ctx, s.indexKey()).Result()
	if err != nil {
		return 0
	}
	return int(n)
}

// Clear removes all wisps from the store.
func (s *redisWispStore) Clear() {
	ctx := context.Background()
	ids, err := s.client.SMembers(ctx, s.indexKey()).Result()
	if err != nil {
		return
	}

	if len(ids) == 0 {
		return
	}

	keys := make([]string, 0, len(ids)+1)
	for _, id := range ids {
		keys = append(keys, s.wispKey(id))
	}
	keys = append(keys, s.indexKey())

	s.client.Del(ctx, keys...)
}

// Close releases any resources held by the store.
func (s *redisWispStore) Close() error {
	s.closed.Store(true)
	s.Clear()
	return s.client.Close()
}

// Compile-time interface check.
var _ WispStore = (*redisWispStore)(nil)
